...
・各種表
　・(無印)
　　　シーン表　ST｜ファンブル表　FT｜感情表　ET
　　　変調表　WT｜戦場表　BT｜異形表　MT｜ランダム特技決定表　RTT
　・(弐)
　　　都市シーン表　CST｜館シーン表　　MST｜出島シーン表　DST
　・(参)
　　　トラブルシーン表　TST｜日常シーン表　NST｜回想シーン表　KST
　・(死)
　　　東京シーン表　TKST｜戦国シーン表　GST
　・(乱)
　　　戦国変調表　GWT｜
　・(リプレイ戦1～2巻)
　　　学校シーン表　GAST｜京都シーン表　KYST｜神社仏閣シーン表　JBST
　・(怪)
　　　怪ファンブル表　KFT｜怪変調表　KWT
　・（その他）
　　　出島EXシーン表　DXST　＞＞　災厄シーン表　CLFT
　　　斜歯ラボシーン表　HLST　＞＞　培養プラントシーン表　PLFT
　　　夏の終わりシーン表　NOST
　・（サーバーオリジナル）
　　　分野指定特技ランダム表
　　　器術　KTT｜体術　TTT｜忍術　NTT｜謀術　BTT｜戦術　STT｜妖術　YTT
・D66ダイスあり
INFO_MESSAGE_TEXT
  end
  ...
  def rollDiceCommand(command)
    ...
    when /([KTNBSY]?TT)/i   # 特技ランダム表
      return sinobigami_skill_table(string)
    when /(MT)/i   # 異形表
  ...
  def sinobigami_wrong_table(string)
    ...
    when /GWT/
      type = '戦国'
      table = [
               '催眠:この変調を受けた者は、戦闘に参加したとき、戦闘開始時、もしくはこの変調を受けた時に【生命力】を1点減少しないと、その戦闘から自動的に脱落する。この効果は累積しない。サイクルの終了時に《意気》の判定を行い、成功するとこの効果は無効化される。',
               '火達磨:この変調を受けた者は、ファンブル値が1上昇し、ファンブルすると1点の接近戦ダメージを受ける。この効果は累積する。シーンの終了時にこの効果は無効化される。',
               '猛毒:この変調を受けた者は、戦闘に参加したとき、ラウンドの終了時にサイコロを1個振る。その目が奇数だったら【生命力】を1点減少する。この効果は累積する。サイクルの終了時に《毒術》の判定を行い、成功するとこの効果は無効化される。',
               '飢餓:この変調を受けた者は、戦闘に参加したとき、ラウンドの終了時にサイコロを1個振る。その目が偶数だったら【生命力】を1点減少する。この効果は累積する。サイクルの終了時に《兵糧術》の判定を行い、成功するとこの効果は無効化される。',
               '残刃:この変調を受けた者は、回復判定や、忍法、背景、忍具の効果によって【生命力】を回復できなくなる（変調を回復することはできる）。この効果は累積しない。サイクルの終了時に《拷問術》の判定を行い、成功するとこの効果は無効化される。',
               '野望:この変調を受けた者は、命中判定に+1、それ以外のあらゆる判定に-1の修正がつく。この効果は累積しない。サイクルの終了時に《憑依術》の判定を行い、成功するとこの効果は無効化される。',
              ]
    when /KWT/
      type = '怪'
      table = [
  ...
  # 特技表
  def sinobigami_skill_table(string)
    type = ""
    table = []
    ...
    when /KTT/i
      type = '器術分野ランダム特技'
      table = ['《絡繰術》','《火術》','《水術》','《針術》','《仕込み》','《衣装術》','《縄術》','《登術》','《拷問術》','《壊器術》','《掘削術》']
    when /TTT/i
      type = '体術分野ランダム特技'
      table = ['《騎乗術》','《砲術》','《手裏剣術》','《手練》','《身体操術》','《歩法》','《走法》','《飛術》','《骨法術》','《刀術》','《怪力》']
    when /NTT/i
      type = '忍術分野ランダム特技'
      table = ['《生存術》','《潜伏術》','《遁走術》','《盗聴術》','《腹話術》','《隠形術》','《変装術》','《香術》','《分身の術》','《隠蔽術》','《第六感》']
    when /BTT/i
      type = '謀術分野ランダム特技'
      table = ['《医術》','《毒術》','《罠術》','《調査術》','《詐術》','《対人術》','《遊芸》','《九ノ一の術》','《傀儡の術》','《流言の術》','《経済力》']
    when /STT/i
      type = '戦術分野ランダム特技'
      table = ['《兵糧術》','《鳥獣術》','《野戦術》','《地の利》','《意気》','《用兵術》','《記憶術》','《見敵術》','《暗号術》','《伝達術》','《人脈》']
    when /YTT/i
      type = '妖術分野ランダム特技'
      table = ['《異形化》','《召喚術》','《死霊術》','《結界術》','《封術》','《言霊術》','《幻術》','《瞳術》','《千里眼の術》','《憑依術》','《呪術》']
    end
    
    
    return get_sinobigami_2d6_scene_table_output("#{type}", table)
  end
...
TABLE_TEXT_END
    ...
    'DXST' => {
      ...
      :type => '2D6',
      :table => <<'TABLE_TEXT_END'
 2:迷宮街。いつから囚われてしまったのだろう？何重にも交差し、曲がりくねった道を歩き続ける。このシーンの登場人物は《記憶術》で判定を行わなければならない。成功すると、迷宮の果てで好きな忍具を一つ獲得する。失敗すると、行方不明の変調を受ける。
...
TABLE_TEXT_END
    ...
    'HLST' => {
      ...
      :type => '2D6',
      :table => <<'TABLE_TEXT_END'
 2:今週のびっくりどっくり斜歯開発室、1D6を振り奇数なら好きな忍具が入手できる。偶数でも好きな忍具が入手できるが、射撃戦ダメージを１点受ける。
...
TABLE_TEXT_END
    ...
    'DEST' => {
      ...
      :type => '2D6',
      :table => <<'TABLE_TEXT_END'
厨房。召使いや下働きたちが忙しなく動いている。
闘技場。今日も闘士や猛獣が血なまぐさい戦いを繰り広げている。
牢獄。地表ぎりぎりに作られた鉄格子から細切れの光が汚れた床に落ちている。
控えの間。無数にある部屋の一つ。壁はモザイクタイルと重いタペストリーで覆われている。
回廊。石造りのアーチに支えられた回廊。あらゆる部屋と中庭を結んでいる。
中庭。無数にある庭の一つ。噴水から流れ出した水路に様々な花が色を映している。
蔵書庫。様々な知、技術、物語が収められている。誰も知らずここに眠るだけの知識も多い。
沐浴場。いつでも涼を求められるように澄んだ水が湛えられている。
塔。ひたすらに螺旋を描く階段の末、最上階には整えられた部屋がある。
礼拝堂。モザイクタイルの壁の向こうで楽士が奏でる音楽が流れている。
天蓋の間。渾天儀が設えてあり、ドーム型の天井を覆うタイルが星を描き出している。
TABLE_TEXT_END
    },
  }

  setPrefixes(['ST', 'FT', 'ET', 'WT', 'BT', 'CST', 'MST', 'IST','EST','DST','TST', 'NST', 'KST', 'TKST', 'GST', 'GWT', 'GAST', 'KYST', 'JBST', 'KFT', 'KWT', 'MT', 'RTT',
  'KTT','TTT','NTT','BTT','STT','YTT',
  'DEST'] + @@tables.keys)
